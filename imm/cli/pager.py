...
# https://pypi.python.org/pypi/pager/3.3
# https://bitbucket.org/techtonik/python-pager

#-------------------------------------------------------------------------------
# This is a fork of project version 3.3 at:
#   https://pypi.python.org/pypi/pager/3.3
#   https://bitbucket.org/techtonik/python-pager
#
# Version 3.3 Modified by E.R. Uber  ==> Version 3.3u
#-------------------------------------------------------------------------------
__version__ = '3.3u'
...
def page(a_list, pagecallback=prompt):
    """
    Convert a_list to an iterator for`content` after determining length of list
    to calculate how many pages of output will paged to the terminal.
    ...
    totalpages = int(math.ceil(len(a_list) / (height-1)))
    ...
    try:
        ...
        pagecallback(pagenum, totalpages)
        return
        ...
        while linesleft:
            ...
            if linelist: # prepare symbols left on the line for the next iteration
                ...
                try:
                    ...
                    #pagecallback(pagenum, totalpages)
                    return
        if pagecallback(pagenum, totalpages) == False:
            return
        pagenum += 1
